...
void *BootstrapAllocator::internal_allocate(const TypeInfo &info, size_t count) {
    return mem::os::calloc(count, info.size);
}

void *BootstrapAllocator::internal_allocate_nonzero(const TypeInfo &info, size_t count) {
    return mem::os::malloc(count * info.size);
}

void *BootstrapAllocator::internal_reallocate(const TypeInfo &info, void *old_ptr, size_t old_count, size_t new_count) {
...
void *BootstrapAllocator::internal_reallocate_nonzero(const TypeInfo &info, void *old_ptr, size_t old_count, size_t new_count) {
    return mem::os::realloc(old_ptr, new_count * info.size);
}

void BootstrapAllocator::internal_deallocate(const TypeInfo &info, void *ptr, size_t count) {
    mem::os::free(ptr);
}
...
void *CAllocator::internal_allocate(const TypeInfo &info, size_t count) {
    return mem::os::calloc(count, info.size);
}

void *CAllocator::internal_allocate_nonzero(const TypeInfo &info, size_t count) {
    return mem::os::malloc(count * info.size);
}

void *CAllocator::internal_reallocate(const TypeInfo &info, void *old_ptr, size_t old_count, size_t new_count) {
    auto new_ptr = this->internal_reallocate_nonzero(info, old_ptr, old_count, new_count);
    if (new_count > old_count)
        memset(reinterpret_cast<uint8_t *>(new_ptr) + (old_count * info.size), 0, (new_count - old_count) * info.size);
    return new_ptr;
...
void *CAllocator::internal_reallocate_nonzero(const TypeInfo &info, void *old_ptr, size_t old_count, size_t new_count) {
    return mem::os::realloc(old_ptr, new_count * info.size);
}

void CAllocator::internal_deallocate(const TypeInfo &info, void *ptr, size_t count) {
    mem::os::free(ptr);
}
...
struct ArenaAllocator::Impl {
    ...
    inline void *allocate(const TypeInfo& info, size_t count);
    inline void *reallocate(const TypeInfo& info, void *old_ptr, size_t old_count, size_t new_count);
...
void *ArenaAllocator::Impl::allocate(const TypeInfo& info, size_t count) {
#ifndef NDEBUG
...
void *ArenaAllocator::Impl::reallocate(const TypeInfo& info, void *old_ptr, size_t old_count, size_t new_count) {
#ifndef NDEBUG
    // make behavior when size == 0 portable
...
void *ArenaAllocator::internal_allocate(const TypeInfo &info, size_t count) {
    return this->impl->allocate(info, count);
}

void *ArenaAllocator::internal_allocate_nonzero(const TypeInfo &info, size_t count) {
    return this->impl->allocate(info, count);
}

void *ArenaAllocator::internal_reallocate(const TypeInfo &info, void *old_ptr, size_t old_count, size_t new_count) {
    return this->internal_reallocate_nonzero(info, old_ptr, old_count, new_count);
}

void *ArenaAllocator::internal_reallocate_nonzero(const TypeInfo &info, void *old_ptr, size_t old_count, size_t new_count) {
    return this->impl->reallocate(info, old_ptr, old_count, new_count);
}

void ArenaAllocator::internal_deallocate(const TypeInfo &info, void *ptr, size_t count) {
    // noop
}
